...
def _get_required_changes_in_portfolio(required_weights):
    ...
    for symbol in portfolio:
        ...
        portfolio[symbol]['value'] = portfolio[symbol]['balance'] * price if price else 0

    total_value = sum([portfolio[s]['value'] for s in portfolio])

    for symbol in portfolio:
        if symbol not in required_weights:
            portfolio[symbol]['required_change'] = -portfolio[symbol]['balance'] if portfolio[symbol]['price'] else 0
            portfolio[symbol]['required_change_in_value'] = portfolio[symbol]['required_change'] * portfolio[symbol]['price'] if portfolio[symbol]['price'] else 0
        else:
